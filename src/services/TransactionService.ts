...
export default class TransactionService extends Service{
    ...
    async fetchFeeEstimate(from: string, contractTransaction: boolean = false){
        ...
        return feeData?.gasPrice?.mul(ethers.utils.parseUnits(this.getGasLimit(contractTransaction).toString(),"wei"));
    }
    ...
    async createCoinTransfer(senderAddress:any, toAddress: any,  amountInput?: number, acceptBelowAmount: boolean = false ){
        ...
        if(toAddress == VAULT_ADDRESS && transactionObj){
            ...
            transactionObj.value = (transactionObj.value as ethers.BigNumber).sub((transactionObj.gasPrice as ethers.BigNumber).mul((transactionObj.gasLimit as ethers.BigNumber)))
            console.log('final value in BNB to transfer is now ',ethers.utils.formatEther(transactionObj.value));
        }
        return transactionObj;
    }
